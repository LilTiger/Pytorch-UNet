...
if __name__ == '__main__':
    imgPredict = cv2.imread('48328.png')
    imgLabel = cv2.imread('label.png')
    # 直接将labelme生成的mask文件（label.png）拿来对比即可 注意此时二者需要resize为相同大小
    imgLabel = cv2.resize(imgLabel, (640, 480))
    # 注意 下面的代码会将predict和mask图片转化为灰度图 因此可以将labelme生成的mask文件直接拿来对比 无需经过binary.py二值化为灰度图
